$(function () {
    // 获取此店铺下的商品列表的URL
    var listUrl = '../shop/awardlistshop?pageIndex=1&pageSize=999';
    // 商品下架URL
    var statusUrl = '../shop/modifyaward';
            ...
            if (data.status) {
                ...
                awardList.map(function (item, index) {
                    ...
                    tempHtml += '' + '<div class="row row-award">'
                        ...
                        + item.enableStatus
                        + '">编辑</a>'
                        + '<a href="#" class="status" data-id="' + item.awardId + '" data-status="'
                        + contraryStatus
                        + '">'
                        + textOp
                        + '</a>'
    ...
    $('.award-wrap').on('click', 'a', function (e) {
        ...
        if (target.hasClass('edit')) {
            ...
            window.location.href = '../shop/awardoperation?awardId=' + e.currentTarget.dataset.id;
            ...
            window.location.href = '../frontend/awarddetails?awardId=' + e.currentTarget.dataset.id;
        }
    });
